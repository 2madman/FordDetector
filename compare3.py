...
def compare_with_folder(target_image_path, folder_path="EdgeDetectedFolder"):
    ...
    if not is_edge_image(target_img):
        target_img = cv2.Canny(target_img, 100, 200)
    ...
    for img_path in folder.glob('*'):
        if img_path.suffix.lower() in valid_extensions:
            try:
                ...
                if not is_edge_image(comp_img):
                    comp_img = cv2.Canny(comp_img, 100, 200)
...
def main():
    target_image = "CompareCars/puma6.jpg"  # Replace with your target image path
